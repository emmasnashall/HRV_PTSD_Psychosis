...
data <- read.csv(paste0(
  "Processed/Neurokit2/Collated//Reprocessed_23_05/FQ_Collated.csv"
))

savefolder <- paste0("Analysis/H1a/FQ/Sensitivity/")
...
#z-score the BoxCox_HRV_HF
data$BoxCox_HRV_HF_z <- as.numeric(scale(data$BoxCox_HRV_HF))

# # Simple linear regression
# lm_hrv_adj <- lm(HRV_HF ~ Heart_Period, data = data)

# # Extract residuals
# data$HRV_HF_adj <- residuals(lm_hrv_adj)

#model_with_pvalues <- lmer(BoxCox_HRV_HF ~ Run_Type + average_quality + (1 | Participant) + (1 | Participant:Run_number), data = data)
model_with_pvalues <- lmer(BoxCox_HRV_HF_z ~ Run_Type + (1 | Participant), 
                          # weights = weights,
                          data = data)
